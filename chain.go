...
// ReadVaultStatus reads only the on-chain status for a vault (1 RPC call).
// Cheaper than ReadVaultStats — used to populate the match list table.
func (c *ChainClient) ReadVaultStatus(vaultAddr string) (uint8, error) {
	addr := common.HexToAddress(vaultAddr)
	val, err := c.readUint8(addr, c.VaultABI, "status")
	if err != nil {
		return 0, err
	}
